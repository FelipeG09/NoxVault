  ...
  Future<void> _submit() async {
    ...
    if (_useBiometricInsteadOfCurrent) {
      ok = await auth.changeMasterPinWithBiometric(
        newPin: newPin,
        confirm: confirm,
        reEncryptNotes: () => vault.reEncryptWithNewKey(auth.vaultKey!),
      );
    } else {
              ...
              children: [
                ...
                if (!_useBiometricInsteadOfCurrent) ...[
                  TextFormField(
                    ...
                    validator: (value) {
                      ...
                      if (p.length < 4 || p.length > 6) return 'PIN inválido.';
                      if (!RegExp(r'^[0-9]+$').hasMatch(p)) return 'Apenas números.';
                      return null;
                    },
                  ),
                ...
                TextFormField(
                  ...
                  validator: (value) {
                    ...
                    if (p.length < 4 || p.length > 6) return 'Entre 4 e 6 dígitos.';
                    if (!RegExp(r'^[0-9]+$').hasMatch(p)) return 'Apenas números.';
                    return null;
                  },
                ),
