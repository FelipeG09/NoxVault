...
/// Tempo em segundos até limpar a senha da área de transferência.
const int kClipboardClearSeconds = 30;
...
class _HomeScreenState extends State<HomeScreen>
  ...
  Future<void> _confirmDelete(BuildContext context, Note note) async {
    ...
    if (ok == true && context.mounted) {
      await vault.deleteNote(note.id, key);
      if (context.mounted) {
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            onSelected: (value) {
              ...
              } else if (value == 'security_analysis') {
                Navigator.of(context).push<void>(
                  MaterialPageRoute<void>(
                    builder: (_) => const SecurityAnalysisScreen(),
                  ),
                );
              }
            },
              ...
              const PopupMenuItem(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 12),
                    Text('Análise de segurança com IA'),
                  ],
                ),
              ),
              const PopupMenuItem(
...
class _NoteCardState extends State<_NoteCard> {
  ...
  /// Exige biometria (quando disponível) antes de revelar a senha.
  Future<void> _revealPassword(BuildContext context) async {
    final auth = context.read<AuthProvider>();
    if (auth.biometricAvailable) {
      final ok = await auth.authenticateForSensitiveAction(
        'Confirme com biometria para revelar a senha',
      );
      if (!ok) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Biometria não reconhecida.')),
          );
        }
        return;
      }
  ...
  /// Copia a senha e agenda limpeza da área de transferência após 30 segundos.
  Future<void> _copyPassword(BuildContext context, String password) async {
    await Clipboard.setData(ClipboardData(text: password));

    _clipboardTimer?.cancel();
    setState(() => _clipboardCountdown = kClipboardClearSeconds);

    _clipboardTimer = Timer.periodic(const Duration(seconds: 1), (t) {
      if (!mounted) {
        t.cancel();
        return;
      }
      setState(() => _clipboardCountdown--);
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Senha copiada. Área de transferência será limpa em ${kClipboardClearSeconds}s.',
          ),
        ),
      );
    }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (note.password.trim().isNotEmpty) ...[
                      Row(
                        children: [
                          ...
                          // Botão revelar — exige biometria
                          IconButton(
                            tooltip: _showPassword ? 'Ocultar senha' : 'Revelar senha',
                            icon: Icon(
                              _showPassword
                            ...
                            onPressed: _showPassword
                                ...
                                : () => _revealPassword(context),
                          ),
                          // Botão copiar — com countdown no tooltip
                          IconButton(
                            tooltip: _clipboardCountdown > 0
                                ? 'Limpa em ${_clipboardCountdown}s'
                                : 'Copiar senha',
                            icon: _clipboardCountdown > 0
                                ? Text(
                                    '${_clipboardCountdown}s',
                                    style: const TextStyle(
                                      fontSize: 11,
                            ...
                            onPressed: () =>
                                _copyPassword(context, note.password),
                          ),
                        ],
                      ),
                      if (pwdAnalysis != null)
                    ...
                    Text(
                      ...
                      '${note.createdAt.year}'
                      '  •  Atualizada em ${note.updatedAt.day.toString().padLeft(2, '0')}/'
